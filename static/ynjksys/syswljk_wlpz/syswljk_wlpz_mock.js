(function(global){
  ...
  function load(){try{var v=JSON.parse(localStorage.getItem(KEY));if(v&&v.types&&v.devices&&v.accounts&&v.areas){v.types=v.types.filter(function(x){return !(x.fields||[]).some(function(f){return f.key==="alarmDelay";});});v.devices=v.devices.filter(function(x){return x.id!=="D-006";});return v;}}catch(e){}return clone(seed);}
