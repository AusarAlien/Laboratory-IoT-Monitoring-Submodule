...
begin
  id := 'ynjksys_hjjc_wsd_jl01q'; name := '环境监测记录明细查询'; direct := '0';
  ...
  select i.instid,trim(i.instno) instno,trim(i.instnm) instnm,trim(i.libseq) libseq,
         i.fhiino,trim(i.fusedptno) fusedptno,trim(i.loaction) loaction
    from htlis.lp_tbc_instfile i cross join ctx c
  ...
  select to_char(m.circslogseq)||':'||to_char(d.circuitemid) frecordid,
         m.circslogseq fcrseq,d.circuitemid,
         ...
         to_char(i.fhiino) fhiino,m.instid fdepotseq,
         ...
         null falarmdesc,m.testtime frecorddt,
         nvl(trim(m.recordman),nvl(trim(m.testman),trim(m.ftestempid))) fempid,
         trim(m.fmode) fremark,trim(m.ftype) fsourcecode,
         trim(m.libseq) flibseq,m.instid finstid
  ...
  select f.*,count(*) over() ftotalcount,
         row_number() over(order by frecorddt desc,fcrseq desc,circuitemid) rn
    from filtered f
)
select rn frowseq,frecordid,fcrseq,fdeviceid,fdeviceip,fdevicename,fdeptno,fhiino,
       fdepotseq,fmetriccode,fmetricname,frawvalue,fvalue,funit,flower,fupper,
       fstatuscode,fstatus,falarmdesc,to_char(frecorddt,'yyyy-mm-dd hh24:mi:ss') frecordtime,
       fempid,fremark,fsourcecode,flibseq,finstid,ftotalcount
  from numbered
 where rn between ((nvl(?,1)-1)*nvl(?,20)+1) and (nvl(?,1)*nvl(?,20))
 order by rn~';
  bsql_pv := 'hiino_sql_equal,start_time_sql_equal,start_time_sql_equal,end_time_sql_equal,end_time_sql_equal,device_sql_equal,device_sql_equal,device_sql_equal,dept_no_sql_equal,dept_no_sql_equal,metric_sql_equal,metric_sql_equal,metric_sql_equal,status_sql_equal,status_sql_equal,status_sql_equal,page_sql_equal,page_size_sql_equal,page_sql_equal,page_size_sql_equal;';
  bsql_pt := 'N,V,V,V,V,V,V,V,V,V,V,V,V,V,V,V,N,N,N,N;';
