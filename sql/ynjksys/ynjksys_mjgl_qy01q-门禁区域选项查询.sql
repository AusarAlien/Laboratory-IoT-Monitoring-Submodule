...
begin
  id := 'ynjksys_mjgl_qy01q'; name := '门禁区域选项查询'; direct := '0';
  ...
  bsql := q'~select trim(l.area) farea,
       max(trim(l.areaalias)) fareaalias,
       max(trim(l.labname)) flabname,
       count(distinct trim(l.sn)) fdevicecount
  from ib_tbs_labarea l
 where trim(l.area) is not null
 group by trim(l.area)
 order by trim(l.area)~';
