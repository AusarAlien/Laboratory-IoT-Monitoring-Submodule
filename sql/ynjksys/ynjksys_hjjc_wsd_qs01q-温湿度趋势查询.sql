...
begin
  id := 'ynjksys_hjjc_wsd_qs01q'; name := '环境监测数值趋势查询'; direct := '0';
...
inst_data as (
  ...
  select f.*,row_number() over(order by frecorddt desc,fcrseq desc,circuitemid) rn from filtered f
)
select frecordid,fcrseq,fdeviceid,fdeviceip,fdevicename,fdeptno,
       fmetriccode,fmetricname,frawvalue,fvalue,funit,fstatuscode,fstatus,
       to_char(frecorddt,'yyyy-mm-dd hh24:mi:ss') frecordtime
  from limited where rn<=2000 order by frecorddt,fcrseq,circuitemid~';
  bsql_pv := 'hiino_sql_equal,start_time_sql_equal,start_time_sql_equal,end_time_sql_equal,end_time_sql_equal,device_sql_equal,device_sql_equal,device_sql_equal,dept_no_sql_equal,dept_no_sql_equal,metric_sql_equal,metric_sql_equal,metric_sql_equal;';
  bsql_pt := 'N,V,V,V,V,V,V,V,V,V,V,V,V;';
