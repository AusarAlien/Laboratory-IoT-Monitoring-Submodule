...
begin
 id:='ynjksys_wljk_nh_jl01q'; name:='物联能耗指标记录查询'; direct:='0'; cndxml:='<?xml version="1.0" encoding="GB2312"?><EprSelect></EprSelect>'; cndxsl:=''; thesql:=''; dispsql:=''; param:=''; cfgxml:=''; resulttype:='ntable'; header:=''; footer:='';
 ...
 select fid||'-VOLTAGE',b.*,'VOLTAGE','电压',fvoltage,'V' from b union all
 select fid||'-POWER',b.*,'POWER','功率',fpowerkw,'kW' from b union all
 select fid||'-ENERGY',b.*,'ENERGY','总能耗',fttlenergy,'kWh' from b union all
 select fid||'-DURATION',b.*,'DURATION','总使用时长',fttlusetimer,'h' from b union all
 ...
 select m.*,case when fvalue is null then '无效' when trim(fstatus)='2' then '超限' else '正常' end fdatastatus,
        case when trim(fstatus)='2' then '设备上报异常状态' end falarmdesc
 from m where (? is null or fappmonitordt>=to_date(?,'yyyy-mm-dd hh24:mi:ss'))
 and (? is null or fappmonitordt<=to_date(?,'yyyy-mm-dd hh24:mi:ss'))
 and (? is null or finstid=?) and (? is null or fdeptno=?)
 and (? is null or fmetriccode=? or fmetricname=?)
